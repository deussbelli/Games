...
int menu_close() {
	...
	while (isMenu2)
	{
		...
		{
			if (menuNum2 == 1) { music.stop(); window.close(); }
			if (menuNum2 == 2) { music.stop(); window.close(); }
...
void menu_open(RenderWindow& window) 
{
	...
	while (isMenu)
	{
		...
		if (Mouse::isButtonPressed(Mouse::Left))
		{
			if (menuNum == 1) { music.stop(); window.display();  lvl_1();  }
			if (menuNum == 2) { music.stop(); window.display();  lvl_3();}
			if (menuNum == 3) { music.stop(); window.display();  lvl_2(); }
